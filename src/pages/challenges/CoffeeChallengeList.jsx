...
function CoffeeChallengeList() {
  ...
  const fetchChallenges = (pageNumber) => {
    const url = "http://localhost:9000/challenges/read/coffee?page="+pageNumber;
    console.log(url);
    console.log(filter);
    axios.post(url, {
      status : filter,
      ...
      setChallengeList(res.data.data.content);
      setTotalElements(res.data.data.totalElements);
      console.log("22222222233333333333");
  ...
  return (
    <Container>
      <Title>커피 챌린지</Title>
      <ButtonDiv><Button onClick={() => navigate(`/challenges/insert/coffee`)}>커피 챌린지 만들기</Button></ButtonDiv>
      <hr />
      <FilterSelector>
        <Select
          id="filterSelect"
      ...
      <Feed>
        {challengeList.map((challenge, index) => (
            ...
            deposit={challenge.savingAmount}
...
const ButtonDiv = styled.div`
  ...
  text-align: right;
`
export default CoffeeChallengeList;